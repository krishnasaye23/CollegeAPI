...
public class TeachersController {
    ...
    public ResponseEntity<Object> getTeachers(){
        try {
            List<TeacherEntity> mo = teachersImpl.getTeachers();
    ...
    public ResponseEntity<Object> getbyid(@PathVariable int teacher_id){
        try {
            TeacherEntity response = teachersImpl.getTeacherbyid(teacher_id);
    ...
    public ResponseEntity<TeacherEntity> addTeachers(@Valid @RequestBody TeacherEntity teachers) {
        TeacherEntity response=teachersImpl.addTeachers(teachers);
        return new ResponseEntity<>(response,HttpStatus.CREATED);
    }
    @PutMapping("/teachers")
    public ResponseEntity<TeacherEntity> updateTeachers(@Valid @RequestBody TeacherEntity teachers) {
        TeacherEntity response=teachersImpl.updateTeachers(teachers);
        return new ResponseEntity<>(response,HttpStatus.CREATED);
    }
    @DeleteMapping("teachers")
    public ResponseEntity<Object> delrecord(@RequestParam("teacher_id") int teacher_id){
        try {
            TeacherEntity response = teachersImpl.delrecord(teacher_id);
